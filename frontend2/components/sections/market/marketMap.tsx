...
export default function MarketMapPage({ initialData = defaultState }: MarketMapProps) {
  ...
  return (
    <SectionLayout title="Market Map" source="Source: 1.PromenadeAI, 2.Crunchbase">
      {segments.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
          {topSegments.map((segment, index) => (
            <div key={index} className="border border-[#ced7db] rounded-lg overflow-hidden shadow-sm">
              ...
              <div className="p-4 bg-white">
                <div className="grid grid-cols-3 gap-4">
                  {segment.companies.slice(0, 12).map((company, companyIndex) => (
                    <div key={companyIndex} className="flex items-center justify-center h-12">
                      {segment.companyLogos && segment.companyLogos[companyIndex] ? (
                        <div className="relative h-8 w-full">
                          <img
                            src={segment.companyLogos[companyIndex] || "/placeholder.svg"}
                            alt={company}
