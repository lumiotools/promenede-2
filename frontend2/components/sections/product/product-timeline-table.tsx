...
export default function ProductTimelineTable({ initialData }: ProductTimelineTableProps) {
    ...
    <SectionLayout title="Product Timeline" sourceText={sourceText}>
      {displayLaunches.length === 0 ? (
        <div className="flex items-center justify-center h-64">
          <p className="text-gray-500 text-lg">No product timeline data available</p>
        </div>
      ) : (
            ...
            <tbody>
              {displayLaunches.map((launch, index) => (
                <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                  <td className="p-4 border-t border-gray-200">{formatDate(launch.date)}</td>
                  <td className="p-4 border-t border-gray-200 font-medium">
                    {launch.product_name || "Unnamed Product"}
                  </td>
                  <td className="p-4 border-t border-gray-200">{launch.description || "No description available"}</td>
                </tr>
              ))}
            </tbody>
          </table>
