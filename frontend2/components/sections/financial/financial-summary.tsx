...
export function FinancialSummary({ initialData }: FinancialSummaryProps) {
  ...
  const processData = (data?: KeyFinancials | null): ChartData[] => {
    ...
    return fiscalYearStatements.map((statement) => {
      const year = statement.period_end_date ? new Date(statement.period_end_date).getFullYear().toString() : ""
      return {
        year,
        revenue: statement.revenue ? Math.round(statement.revenue / 1000000000) : 0, // Convert to billions
        cost: statement.cost_of_goods_sold ? Math.round(statement.cost_of_goods_sold / 1000000000) : 0, // Convert to billions
        profit: statement.gross_profit ? Math.round(statement.gross_profit / 1000000000) : 0, // Convert to billions
      }
    })
  }
  ...
  const [editingData, setEditingData] = useState<ChartData[]>([])
  ...
  const handleInputChange = (index: number, field: keyof ChartData, value: string) => {
  ...
  return (
    <SectionLayout title="Financial Summary (Past three years)" sourceText={sourceText} showEditButton={false}>
      <div className="flex justify-between items-center mb-4">
        <div className="flex-1"></div> {/* Empty div for spacing */}
        <div className="flex gap-2">
          {!isEditing ? (
            <>
              <Button variant="outline" onClick={handleEditClick} className="border-[#ced7db] text-[#445963]">
                  ...
                  <div className="grid gap-4 py-4">
                    ...
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleNewDataChange("revenue", e.target.value)}
                        className="col-span-3"
                      />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleNewDataChange("cost", e.target.value)}
                        className="col-span-3"
                      />
                    </div>
                    <div className="grid grid-cols-4 items-center gap-4">
                      ...
                      <Input
                        ...
                        onChange={(e) => handleNewDataChange("profit", e.target.value)}
                        className="col-span-3"
                      />
                    </div>
                  </div>
                  <div className="flex justify-end">
                    <Button onClick={handleAddNewDataPoint} className="bg-[#001e69] hover:bg-[#001e69]/90">
                      Add
                    </Button>
                  </div>
                </DialogContent>
            ...
            <>
              <Button variant="outline" onClick={handleCancelClick} className="border-[#ced7db] text-[#445963]">
                <X className="h-4 w-4 mr-2" />
                Cancel
              </Button>
              <Button onClick={handleSaveClick} className="bg-[#001e69] hover:bg-[#001e69]/90">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <Card className="bg-[#f7f9f9] border-[#ced7db] p-2">
          <CardHeader>
          ...
          <CardContent>
            {isEditing ? (
              <div className="space-y-4">
                {editingData.map((data, index) => (
                  <div key={index} className="grid grid-cols-2 gap-2">
                    <Input value={data.year} disabled className="text-sm bg-[#f2f4f7]" />
                    <Input
                      type="number"
                      value={data.cost}
