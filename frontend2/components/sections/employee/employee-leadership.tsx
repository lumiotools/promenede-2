...
export function LeadershipExecutivesPage({ initialData }: LeadershipExecutivesProps) {
  ...
  const [editingExecutive, setEditingExecutive] = useState<LeadershipExecutive | null>(null)
  ...
  const handleAddAchievement = () => {
    if (editingExecutive && newAchievement.trim()) {
      setEditingExecutive({
        ...editingExecutive,
        achievements: [...(editingExecutive.achievements || []), newAchievement],
      })
      setNewAchievement("")
    }
  }
  ...
  if (loading) {
    return (
      <SectionLayout title="Leadership & Executives">
    ...
    >
      {({ isEditing, editData, setEditData }) => (
        <div className="h-full overflow-auto">
          ...
          <div className="flex justify-end mb-2">
            <Dialog>
              ...
              <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                  <DialogTitle>{editingIndex === -1 ? "Add New Executive" : "Edit Executive"}</DialogTitle>
                </DialogHeader>
                {editingExecutive && (
                  <div className="grid gap-3 py-3">
                    ...
                    <div className="grid grid-cols-4 items-center gap-3">
                      ...
                      <Input
                        ...
                        onChange={(e) => setEditingExecutive({ ...editingExecutive, since: e.target.value })}
                        className="col-span-3 h-8 text-xs"
                    ...
                    <div className="flex justify-end gap-2 mt-3">
                      ...
                      <Button
                        ...
                        onClick={editingIndex === -1 ? handleAddNewExecutive : handleSave}
                      >
          ...
          <div className="mb-3">
            <h3 className="text-sm font-medium text-[#445963] mb-2">Company Tenure</h3>
            <div className="grid grid-cols-3 gap-2">
              <Card className="bg-[#f2f4f7] border-[#ced7db]">
                <CardHeader className="p-2">
                  <CardTitle className="text-center text-[#445963] text-xs font-medium">Average Tenure</CardTitle>
                </CardHeader>
                <CardContent className="p-2 flex justify-center items-center">
                  <div className="text-xl font-bold text-[#156082]">
                    {calculateAverageTenure()} <span className="text-xs">years</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-[#f2f4f7] border-[#ced7db]">
                <CardHeader className="p-2">
                  <CardTitle className="text-center text-[#445963] text-xs font-medium">Executive Count</CardTitle>
                </CardHeader>
                <CardContent className="p-2 flex justify-center items-center">
                  <div className="text-xl font-bold text-[#156082]">
              ...
              <Card className="bg-[#f2f4f7] border-[#ced7db]">
                <CardHeader className="p-2">
                  <CardTitle className="text-center text-[#445963] text-xs font-medium">Combined Experience</CardTitle>
                </CardHeader>
                <CardContent className="p-2 flex justify-center items-center">
                  <div className="text-xl font-bold text-[#156082]">
                    {calculateAverageTenure() * executives.length} <span className="text-xs">years</span>
                  </div>
                </CardContent>
              </Card>
            </div>
          ...
          {/* Executives List */}
          <div>
            <h3 className="text-sm font-medium text-[#445963] mb-2">Leadership Team</h3>

            {executives.length === 0 ? (
              <div className="bg-white rounded-lg p-3 text-center border border-[#ced7db]">
                <p className="text-[#57727e] text-xs">No executives data available</p>
                <Button onClick={handleAddExecutive} className="mt-2 bg-[#156082] hover:bg-[#092a38] text-xs h-7">
                  <PlusCircle className="mr-1 h-3 w-3" />
                  Add Executive
                </Button>
              </div>
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex items-center mt-1 space-x-1">
                            ...
                            {executive.since && (
                              <span className="text-[10px] text-[#57727e]">Since {executive.since}</span>
                            )}
                          </div>
                        </div>
                        <div className="flex space-x-1">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => toggleExpand(index)} className="h-6 w-6 p-0">
                      ...
                      <CardContent className="p-2">
                        <div className="space-y-2 text-xs">
                          {executive.background && (
                            <div>
                              <h4 className="font-medium text-[#445963] text-[11px]">Background</h4>
                              <p className="text-[#57727e] text-[10px]">{executive.background}</p>
                            </div>
                          )}

                          {executive.achievements && executive.achievements.length > 0 && (
                            <div>
                              <h4 className="font-medium text-[#445963] text-[11px]">Key Achievements</h4>
                              <ul className="list-disc pl-4 space-y-0.5">
                                {executive.achievements.slice(0, 3).map((achievement, idx) => (
                                  <li key={idx} className="text-[#57727e] text-[10px]">
                                    {achievement}
                                  </li>
                                ))}
                                {executive.achievements.length > 3 && (
                                  <li className="text-[#57727e] text-[10px] italic">
                                    +{executive.achievements.length - 3} more achievements
                                  </li>
                                )}
                              </ul>
                            </div>
                          ...
                          {executive.educationalBackground && (
                            <div>
                              <h4 className="font-medium text-[#445963] text-[11px]">Education</h4>
                              <p className="text-[#57727e] text-[10px]">{executive.educationalBackground}</p>
                            </div>
                          )}
                        </div>
                      </CardContent>
