...
import { Linkedin, Edit, Plus, X, Check } from "lucide-react"
...
export function EmployeeKeyMembers({ initialData = defaultState }: EmployeeKeyMembersProps) {
  ...
  const handleChange = (id: number | null, field: keyof KeyMember, value: string) => {
    if (id === null) {
      // Updating new member
      setNewMember((prev) => ({
        ...prev,
      ...
      setMembers((prev) => prev.map((member) => (member.member_id === id ? { ...member, [field]: value } : member)))
    }
  }
  ...
  // Limit to top 5 members for display
  const displayMembers = currentMembers.slice(0, 5)
  ...
  return (
    <SectionLayout title="Organization: Key Members">
      ...
      <div className="flex justify-between items-center mb-4">
        <Button onClick={handleAddNew} className="bg-[#002169] hover:bg-[#001a54]" disabled={isAdding}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {displayMembers.length > 0 ? (
          ...
          displayMembers.map((member) => (
            <div key={member.member_id ?? "undefined-key"} className="border border-[#e5e7eb] p-4 rounded-lg shadow-sm">
              {editingId === member.member_id ? (
                // Edit Mode
                <div className="space-y-3">
                  <Input
                    value={member.member_full_name || ""}
                    onChange={(e) => handleChange(member.member_id, "member_full_name", e.target.value)}
                    placeholder="Full Name"
                    className="w-full"
                  />
                  <Input
                    value={member.member_position_title || ""}
                    onChange={(e) => handleChange(member.member_id, "member_position_title", e.target.value)}
                ...
                <div className="flex justify-between">
                  <div>
                    ...
                    <p className="text-sm text-[#475467] mt-1">{member.member_position_title || "No Position"}</p>
                  </div>
                  <div className="flex space-x-2">
                    ...
                    <a href="#" className="p-1 hover:bg-gray-100 rounded-full transition">
          ...
          <>
            <div className="border border-[#e5e7eb] p-6 rounded-lg shadow-sm flex flex-col items-center justify-center h-32">
              <p className="text-center text-[#8097a2] font-medium">No data present</p>
            </div>
            <div className="border border-[#e5e7eb] p-6 rounded-lg shadow-sm flex flex-col items-center justify-center h-32">
              <p className="text-center text-[#8097a2] font-medium">No data present</p>
            </div>
          ...
          <div className="border border-[#e5e7eb] p-4 rounded-lg shadow-sm border-dashed">
            <div className="space-y-3">
              <Input
                ...
                onChange={(e) => handleChange(null, "member_full_name", e.target.value)}
                placeholder="Full Name"
                className="w-full"
              />
              <Input
                value={newMember.member_position_title || ""}
                onChange={(e) => handleChange(null, "member_position_title", e.target.value)}
                placeholder="Position Title"
                className="w-full"
              />
              <div className="flex justify-end space-x-2 mt-2">
