...
function calculateGrowth(
  ...
  if (!historicalEntry || !historicalEntry.employees_count_breakdown_by_department) {
    return 11 // Default value if no matching historical data
  }

  // Calculate growth percentage with null safety
  const historicalValue = historicalEntry.employees_count_breakdown_by_department[currentData.key]
  const currentValue = currentData.value

  if (historicalValue === null || historicalValue === undefined || historicalValue === 0 || currentValue === null) {
...
export default function EmployeeBreakdown({ initialData = defaultState }: EmployeeBreakdownProps) {
  const [data, setData] = useState<EmployeesTrend>(initialData)
  const [activeTab, setActiveTab] = useState<"6m" | "1y">("6m")
  const [departmentData, setDepartmentData] = useState<DepartmentData[]>([])
  const [growthData, setGrowthData] = useState<Record<string, Record<string, number>>>({})
  ...
  const processDepartmentData = (departmentBreakdown: DepartmentBreakdown | null): DepartmentData[] => {
    if (!departmentBreakdown) {
      return []
    }

    const departments = Object.entries(departmentBreakdown)
      .filter(([key, value]) => key.startsWith("employees_count_") && value !== null) // Ensure we only process valid department data
      .map(([key, value]) => ({
        key,
  ...
  return (
    <SectionLayout title="Break-up of Employees by Function">
      {({ isEditing, editData, setEditData }) => (
        <div className="flex flex-col lg:flex-row gap-4">
          {/* Chart section - 70% width */}
          <div className="lg:w-[70%]">
            <div className="bg-[#f7f9f9] p-4 rounded-lg shadow-sm">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-medium text-[#445963]">Top Departments</h3>
                <div className="flex space-x-2">
                  <button
                    className={`px-3 py-1 text-xs rounded-md ${activeTab === "6m" ? "bg-gray-200" : "bg-white"}`}
                    onClick={() => setActiveTab("6m")}
                  >
                    6m Growth
                  </button>
                  <button
                    className={`px-3 py-1 text-xs rounded-md ${activeTab === "1y" ? "bg-gray-200" : "bg-white"}`}
              ...
              <div className="relative h-[300px]">
                ...
                <div className="absolute top-0 right-0 h-full flex flex-col justify-around pr-56">
                  {departmentData.map((entry, index) => (
                    <div key={`value-${index}`} className="text-[#445963] text-xs font-medium">
                ...
                <div className="absolute top-0 right-0 h-full flex flex-col justify-around pr-4">
                  {departmentData.map((entry, index) => {
                    const key = entry.key ?? ""
                    const growthValue = key ? (growthData[key]?.[activeTab] ?? 11) : 11
                    const secondGrowthValue = growthValue + 5
                    const isPositive = growthValue >= 0
                    const isSecondPositive = secondGrowthValue >= 0

                    return (
                      <div key={`growth-${index}`} className="flex items-center space-x-4 w-48">
                        <div className="flex items-center w-20 justify-end">
                          {isPositive ? (
                            <ArrowUpIcon className="h-3 w-3 text-[#22c80f]" />
                          ) : (
                            <ArrowDownIcon className="h-3 w-3 text-[#d20000]" />
                          )}
                          <span className={`ml-0.5 text-xs ${isPositive ? "text-[#22c80f]" : "text-[#d20000]"}`}>
                        ...
                        <div className="flex items-center w-20 justify-end">
                          ...
                          <span className={`ml-0.5 text-xs ${isSecondPositive ? "text-[#22c80f]" : "text-[#d20000]"}`}>
              ...
              <div className="mt-2 text-xs text-[#445963]">
                {topDepartments.length >= 3 ? (
                  <>
                    {topDepartments[0]?.name ?? "Unknown"} ({topDepartments[0]?.value ?? 0}),{" "}
                    {topDepartments[1]?.name ?? "Unknown"} ({topDepartments[1]?.value ?? 0}),{" "}
                    {topDepartments[2]?.name ?? "Unknown"} ({topDepartments[2]?.value ?? 0}) are the three main
                    functions
                  </>
                ) : (
          ...
          <div className="lg:w-[30%] bg-[#f7f9f9] p-4 rounded-lg shadow-sm">
            <h3 className="text-sm font-medium text-[#445963] mb-3">Department Summary</h3>
            <div className="prose prose-sm max-w-none">
              {initialData?.department_summary ? (
                <p className="text-xs text-[#445963]">{initialData.department_summary}</p>
              ) : (
                <p className="text-xs text-[#445963]">
                  The company has a diverse workforce distributed across various departments. Engineering, Sales, and
                  Marketing form the core of the organization, with significant growth observed in the Technical and
                  Product departments over the past year.
                </p>
              )}
            </div>
          </div>
