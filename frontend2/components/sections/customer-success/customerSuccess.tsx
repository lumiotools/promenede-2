...
const formatMetricValue = (value: string | number | null, prefix?: string, suffix?: string): string => {
...
const CustomerSuccessPage: React.FC = () => {
  ...
  const updateMetric = (id: string, field: keyof MetricCard, value: string | number | null): void => {
  ...
  const updateCaseStudy = (id: string, field: keyof CaseStudy, value: string | string[] | null): void => {
    const newData = { ...editData }
    const caseStudyIndex = newData.caseStudies.findIndex((study) => study.id === id)
  ...
  const updateCaseStudyResult = (caseStudyId: string, resultIndex: number, value: string): void => {
    const newData = { ...editData }
    const caseStudyIndex = newData.caseStudies.findIndex((study) => study.id === caseStudyId)

    if (caseStudyIndex !== -1 && newData.caseStudies[caseStudyIndex].results) {
      const newResults = [...(newData.caseStudies[caseStudyIndex].results || [])]
  ...
  const removeCaseStudyResult = (caseStudyId: string, resultIndex: number): void => {
    const newData = { ...editData }
    const caseStudyIndex = newData.caseStudies.findIndex((study) => study.id === caseStudyId)
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 bg-white">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-gray-700 text-5xl font-normal">Customer Success & Strategic Growth</h1>
        {!isEditing ? (
          <button
        ...
        {isEditing ? (
          <>
            {editData.metrics.map((metric) => (
              <div key={metric.id} className="p-6 bg-gray-50 rounded-lg relative">
                <button
        ...
        {isEditing ? (
          <>
            {editData.caseStudies.map((caseStudy) => (
              <div key={caseStudy.id} className="p-6 bg-gray-50 rounded-lg mb-4 relative">
                <button
                ...
                <div className="mb-4">
                  ...
                  <input
                    ...
                    onChange={(e) => updateCaseStudy(caseStudy.id, "companyName", e.target.value)}
                  />
                </div>
                <div className="mb-4">
                  ...
                  {(caseStudy.results || []).map((result, idx) => (
                    <div key={idx} className="flex items-center mb-2">
                      <input
                        ...
                        value={result}
                        onChange={(e) => updateCaseStudyResult(caseStudy.id, idx, e.target.value)}
                      />
                      <button
                        type="button"
                ...
                <div className="mb-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateCaseStudy(caseStudy.id, "additionalInfo", e.target.value)}
                    rows={3}
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => updateCaseStudy(caseStudy.id, "link", e.target.value)}
                  ...
                  <div className="lg:w-2/3">
                    <h2 className="text-xl font-medium text-gray-700 mb-2">
                      ...
                      <span className="text-gray-600 font-normal ml-2">
                        ...
                        {caseStudy.link && (
                          <a href={`#${caseStudy.link}`} className="text-blue-700 ml-1 underline">
                            {caseStudy.link}
                          </a>
                        )}
                      </span>
                    ...
                    {caseStudy.additionalInfo && <p className="mt-4 text-gray-600">{caseStudy.additionalInfo}</p>}
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="mt-8 text-gray-500 text-sm">Source: 1.PromenadeAI, 2.Crunchbase</div>
    </div>
  )
}
