...
export function FinancialSummary() {
  ...
  const [tableData, setTableData] = useState<{
    headers: string[]
    rows: { category: string; isMainCategory: boolean; values: (string | number | null)[] }[]
  }>({
    headers: [],
    rows: [],
  })
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        } else if (jsonData.data?.company_profile?.key_financials?.income_statements) {
          financialSummary = jsonData.data.company_profile.key_financials.income_statements
        }

        if (financialSummary.length > 0) {
          // Process data for charts
          const fiscalYears = financialSummary
            .filter((statement: FinancialStatement) => statement.period_type === "fiscal_year")
            .sort(
          ...
          if (fiscalYears.length > 0) {
            ...
            const revenueData = {
              labels: fiscalYears.map((year: FinancialStatement) => year.period_end_date.substring(0, 4)),
              values: fiscalYears.map((year: FinancialStatement) => Math.round(year.revenue / 1000000000)),
            }

            const costData = {
              labels: fiscalYears.map((year: FinancialStatement) => year.period_end_date.substring(0, 4)),
              values: fiscalYears.map((year: FinancialStatement) => Math.round(year.cost_of_goods_sold / 1000000000)),
            }

            const profitData = {
              labels: fiscalYears.map((year: FinancialStatement) => year.period_end_date.substring(0, 4)),
              values: fiscalYears.map((year: FinancialStatement) => Math.round(year.net_income / 1000000000)),
            }
          ...
          .filter(
            (statement: FinancialStatement) =>
              statement.period_type.startsWith("q") && statement.period_end_date.startsWith("2024"),
          )
          .sort(
            (a: FinancialStatement, b: FinancialStatement) =>
              new Date(a.period_end_date).getTime() - new Date(b.period_end_date).getTime(),
          ...
          if (q2024Data.length > 0) {
            ...
            const rows = [
              ...
              // S&M section
              {
                category: "S&M",
                isMainCategory: true,
                values: q2024Data.map((q) => formatCurrency(q.total_operating_expense * 0.4)), // Approximation for S&M
              },
              {
                category: "Marketing",
                isMainCategory: false,
                values: q2024Data.map((q, i) => (i === 0 ? formatCurrency(q.total_operating_expense * 0.2) : "-")), // Only first quarter has marketing data
              },
              ...
              // COGS section
              {
                category: "COGS",
                isMainCategory: true,
                values: q2024Data.map((q) => formatCurrency(q.cost_of_goods_sold)),
              },
              {
                category: "Product Purchase",
                isMainCategory: false,
                values: q2024Data.map((q) => formatCurrency(q.cost_of_goods_sold * 0.85)), // Approximation for product purchase
              },
              {
                category: "Packaging",
                isMainCategory: false,
                values: q2024Data.map((q) => formatCurrency(q.cost_of_goods_sold * 0.15)), // Approximation for packaging
              },
              ...
              // Others section
              {
                category: "Others",
                isMainCategory: true,
                values: q2024Data.map((_, i) => (i % 2 === 0 ? "-" : formatCurrency(353000))), // Alternating values
              },
            ]
