...
import { CompanyProfiles, Firmographic, KeyFinancials } from "@/types/company"
import { Edit, Save, X } from "lucide-react"
import { useEffect, useState } from "react"

// Define the types properly to match the actual data structure

// Update the CompanyProfileProps type to accept both the expected structure and the actual structure
type CompanyProfileProps = {
  initialData?: CompanyProfiles
}

// Update the component to handle both data structures
export default function CompanyProfile({ initialData }: CompanyProfileProps) {
  // Transform the data if needed to ensure consistent structure
  const transformData = (data: CompanyProfiles): CompanyProfiles => {
    if (!data) return createEmptyCompanyProfile()

    // Check if data already has the expected structure
    if (data.data && data.data.company_profile) {
      return data
    }
  ...
  const createEmptyCompanyProfile = (): CompanyProfiles => ({
    data: {
      company_profile: {
  ...
  const [data, setData] = useState<CompanyProfiles | undefined>(initialData ? transformData(initialData) : undefined)
  const [editData, setEditData] = useState<CompanyProfiles | undefined>(
    initialData ? transformData(initialData) : undefined,
  )
  ...
  if (!data) {
    // Initialize with empty data structure instead of showing error
    const data = createEmptyCompanyProfile()
  }
  ...
  const keyFinancials = companyProfile.key_financials || createEmptyKeyFinancials()
  ...
  const q2024Data = incomeStatements
    ...
    .sort((a, b) => {
      ...
      return new Date(a.period_end_date).getTime() - new Date(b.period_end_date).getTime()
    })

  // Calculate total for 2024 quarters with null checks
  const q2024Revenue = q2024Data.reduce((sum, quarter) => sum + (quarter.revenue || 0), 0)
  const q2024OperatingProfit = q2024Data.reduce((sum, quarter) => sum + (quarter.ebit || 0), 0)
  const q2024NetIncome = q2024Data.reduce((sum, quarter) => sum + (quarter.net_income || 0), 0)
  ...
  const totalShares = shareholders.reduce((sum, shareholder) => sum + (shareholder.shares || 0), 0)
  const totalPercentage = shareholders.reduce((sum, shareholder) => sum + (shareholder.percentage || 0), 0)
  ...
  const updateFirmographicField = (field: string, value: string | number) => {
    if (!editData || !editData.data || !editData.data.company_profile || !editData.data.company_profile.firmographic)
      return
  ...
  const updateShareholderField = (index: number, field: keyof Shareholder, value: string | number) => {
    if (!editData || !editData.data || !editData.data.company_profile || !editData.data.company_profile.shareholders)
      return
  ...
  return (
    <div className="w-full max-w-full bg-white">
      ...
      <div className="border-t border-[#e5e7eb] mb-6">{firmographic.legal_name || "N/A"}</div>
        ...
        <div className="flex flex-col gap-6">
                    ...
                    {isEditing.firmographic ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          className="w-full p-1 border border-gray-300 rounded"
                          value={fiscalYears.length > 0 ? fiscalYears[0].revenue || 0 : 0}
                          disabled
                        />
                  ...
                  <td className="py-3 px-4 border-t border-[#e5e7eb] text-black">
                    {isEditing.firmographic ? (
                      <input
                        type="number"
                        className="w-full p-1 border border-gray-300 rounded"
                        value={editData?.data?.company_profile?.firmographic?.employees_count || 0}
                      ...
                      <textarea
                        ...
                        onChange={(e) => {
                          ...
                          const newProducts = values.map((value, i) => ({
                            uuid: (editData?.data?.company_profile?.firmographic?.products_services?.[i]?.uuid ||
                              `new-${i}`) as string,
                            value,
                            image_id: null,
                            permalink: null,
                          ...
                          const newEditData = { ...editData }
                          if (newEditData?.data?.company_profile?.firmographic) {
          ...
          <div className="border border-[#e5e7eb] rounded-md overflow-hidden">
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {(isEditing.shareholders && editData?.data?.company_profile?.shareholders
                    ...
                    <td className="py-3 px-4 border-t border-r border-[#e5e7eb] text-black">
                      {isEditing.shareholders ? (
                        <input
                          ...
                          onChange={(e) =>
                            updateShareholderField(index, "shares", Number.parseInt(e.target.value) || 0)
                          }
                        />
                      ) : (
                    ...
                    <td className="py-3 px-4 border-t border-r border-[#e5e7eb] text-black">
                      {isEditing.shareholders ? (
                        <input
                          ...
                          onChange={(e) =>
                            updateShareholderField(index, "percentage", Number.parseFloat(e.target.value) || 0)
                          }
                        />
                      ) : (
